...
public class Game {
    ...
    private int checkpointsmoved = 0;
    ...
    private void rebootPlayer(Player curr) {
        ...
        List<BoardElement> rebootList = getListOf("RestartPoint");
        if (rebootList.isEmpty()) {
            //ToDO error
            return;
        }
        RestartPoint nearestRestartPoint = (RestartPoint) rebootList.get(0);
        int minDist = calculateDistance(curr.getRobot().getPosition(), nearestRestartPoint.getPosition());
        for (BoardElement elem :
                rebootList) {
            int currDist = calculateDistance(curr.getRobot().getPosition(), elem.getPosition());
            if (currDist < minDist) {
                minDist = currDist;
        ...
        for (BoardElement laser : lasers) {
            ...
            while (pos.getX() < gameMap.getWidth() && pos.getY() < gameMap.getLength()
                ...
                if (gameMap.hasAntenna(pos)) {
                    laserHit = true;
                } else if (getNextPos(pos, laser1.getDirection()).getX() < gameMap.getWidth()
                        && getNextPos(pos, laser1.getDirection()).getY() < gameMap.getLength()
                        && getNextPos(pos, laser1.getDirection()).getX() >= 0
                        && getNextPos(pos, laser1.getDirection()).getY() >= 0) {
                    if (gameMap.nextPosBlocksLaser(getNextPos(pos, laser1.getDirection()), laser1.getDirection())) {
                        laserHit = true;
                    }
                    pos = getNextPos(pos, laser1.getDirection());
